...
- **Hosted MCP server.** For agent frameworks that speak the Model Context Protocol, Bravado hosts a read-only analytics server at `https://mcp.bravadotrade.com/mcp`, authorized with OAuth 2.1. See the [MCP server reference](/products/mcp-server).
...
def get_position(symbol: str):
    ...
    r = http.get(f"{API}/v2/trade/positions", headers=H, params={"symbol": symbol})
    r.raise_for_status()
    return r.json()
```
