...
description: "Build a copy-trading or social trading product on prediction markets with the Bravado Copytrade API."
icon:
  name: "users-round"
  library: "lucide"
---
...
leaderboard = http.get(f"{API}/leaderboard?window=30d&order=pnl&limit=1", headers=H).json()
leader = leaderboard["traders"][0]["address"]

# 2. Subscribe with proportional sizing and a per-fill cap.
sub = http.post(f"{API}/v2/trade/copytrade/subscriptions", headers={**H, **new_key()}, json={
