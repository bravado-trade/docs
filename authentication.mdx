...
| Market-data WebSockets | HMAC-signed upgrade with `stream.read`; see [trade feed](/api/market-data/trade-feed) and [order books](/api/market-data/venue-order-books) |

## Getting your API key

1. [Create an account](https://console.bravadotrade.com/signup) or [log in](https://console.bravadotrade.com/login) to the Bravado Console, your developer dashboard.
...
See [request signing examples](/authentication-examples) for reusable Python and Node.js helpers.
...
| `stream.read` | Connect to authenticated trade and order-book streams, subject to organisation plan and venue access |
...
3. It opens the discovered Bravado authorization endpoint for sign-in and consent. Start setup from [MCP in the Console](https://console.bravadotrade.com/endpoints/mcp).
4. It receives a short-lived access token plus a rotating refresh token.

Tokens are scoped to `mcp.read` and `mcp.query` — the **MCP Server** product, which is separate from the Data API — and bound to the organisation you approve in Console. They are audience-bound to the MCP server, so a token issued for it cannot be replayed against any other Bravado surface.

Revoke a connection from [**Connected applications**](https://console.bravadotrade.com/settings/apps) in Console at any time. Revocation takes effect immediately for the refresh token; an access token already issued is stateless and expires within five minutes.
