...
def leaderboard(metric="pnl", window="30d", limit=50):
    ...
    data = http.get(f"{BASE}{path}", headers=H,
...
def profile(address):
    return {
        "summary":    http.get(f"{BASE}/traders/{address}", headers=H).json(),
        "pnl":        http.get(f"{BASE}/traders/{address}/pnl", headers=H).json(),
        "positions":  http.get(f"{BASE}/traders/{address}/positions/active", headers=H).json(),
        "categories": http.get(f"{BASE}/traders/{address}/categories", headers=H).json(),
    }
```
...
def all_trades(address):
    ...
    while True:
        ...
        page = http.get(f"{BASE}/traders/{address}/trades",
...
```python
def budget():
    acct = http.get(f"{BASE}/v2/trade/account", headers=H).json()
...
```python
r = http.get(f"{BASE}/traders/{address}/statements", headers=H)
