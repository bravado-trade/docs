...
import { bravadoFetch } from "./bravado-auth.mjs";
...
export async function placeOrder(payload: object, attempts = 4) {
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      const res = await bravadoFetch(`${BASE}/v2/trade/order`, {
        method: "POST",
        headers: {
...
```python
http.post(f"{BASE}/v2/trade/users",
...
```bash
curl $BASE/v2/trade/account -H "X-BRAVADO-API-KEY: $BRAVADO_API_KEY" \
  -H "X-BRAVADO-TIMESTAMP: <unix-ms>" \
  -H "X-BRAVADO-SIGNATURE: <signature-for-this-request>"
