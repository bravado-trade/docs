...
```bash
export BRAVADO_MASTER_KEY="your-master-public-key"
export BRAVADO_MASTER_SECRET="your-master-secret"
...
curl $BASE/v2/trade/account \
  -H "X-BRAVADO-API-KEY: $BRAVADO_MASTER_KEY" \
...
MASTER = BravadoAuth(os.environ["BRAVADO_MASTER_KEY"], os.environ["BRAVADO_MASTER_SECRET"])
...
def provision(user_id: str):
    ...
    r = http.post(
        ...
        auth=MASTER,
        headers={"Idempotency-Key": f"provision:{user_id}"},
...
```python
from bravado_auth import session as http, BravadoAuth
def budget_for(user_key: str, user_secret: str) -> int:
    acct = http.get(f"{BASE}/v2/trade/account",
                        auth=BravadoAuth(user_key, user_secret)).json()
...
```python
from bravado_auth import session as http, BravadoAuth
def portfolio(user_key: str, user_secret: str):
    auth = BravadoAuth(user_key, user_secret)
    return {
        "balances":  http.get(f"{BASE}/v2/trade/balances",  auth=auth).json(),
        "positions": http.get(f"{BASE}/v2/trade/positions", auth=auth).json(),
        "activity":  http.get(f"{BASE}/v2/trade/activity",  auth=auth).json(),
    }
```
