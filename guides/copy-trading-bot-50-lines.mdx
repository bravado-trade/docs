...
- A Bravado API key from the [Bravado Console](https://console.bravadotrade.com/keys)
- Scopes: `trade.read` and `trade.execute`
- `pip install requests`
- USDC collateral in your Bravado wallet, but only for the live step at the end

```bash
export BRAVADO_API_KEY="your-public-key"
export BRAVADO_API_SECRET="your-secret"
```
...
```python
from bravado_auth import session as http, BravadoAuth
...
def read(path, **params):
    r = http.get(f"{BASE}{path}", headers=H, params=params or None, timeout=15)
    r.raise_for_status()
    return r.json()
