...
The initial `book` snapshot arrives **either as a JSON array containing one book object, or as a bare object**, and you must handle both. The array form comes straight from the venue; the bare object is the replay Bravado sends when you subscribe to an asset it is already streaming, so you do not sit empty waiting for the next update. Normalise with `Array.isArray(msg) ? msg : [msg]`, as the samples above do.
