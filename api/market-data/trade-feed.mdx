...
The legacy public `/trade-feed/ws` URL remains available. It has no verified
organisation identity, so its messages do **not** appear in Console or Admin
usage and do not consume organisation credits. Adding a key header to that
public URL does not enable attribution.
...
## Legacy public connection (no organisation tracking)
...
The authenticated route sends heartbeat pings; clients must respond with protocol pongs (most libraries do this automatically). For the legacy public route, send client-side protocol pings to keep idle connections alive. Neither route accepts subscription commands. If the socket drops, reconnect to receive a fresh `snapshot`; authenticated reconnects require a fresh timestamp and signature.
