...
`404`. Venue-neutral routes (users, combos, strategies) have no
...
| `PEGGED` | `record_id` | Managed pegged-order strategy. |
| `ICEBERG` | `record_id` | Managed sliced-display strategy. |

Order types not listed (`STOP_LOSS`, `TAKE_PROFIT`, `TRAILING_STOP`, `TRAILING_BUY`) are not yet available on Predict.fun and return `VENUE_CAPABILITY_UNSUPPORTED`.

## Venue-specific behaviour

- **`symbol`** is the on-chain outcome token id (a numeric string), not the market.
- **`price`** is a decimal probability snapped to a **`0.01` tick**. `time_in_force` (IOC/FOK) is not supported. Expiration is supported.
...
- IOC/FOK time-in-force and brackets.
