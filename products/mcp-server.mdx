...
Bravado exposes a hosted **Model Context Protocol (MCP)** server so agent frameworks can call Bravado tools directly. Point an MCP-aware client at it, authorize your Bravado organisation, and the agent can read leaderboards, trader profiles, PnL series, positions and the warehouse behind them.
...
New to Bravado? [Create an account in the Console](https://console.bravadotrade.com/) and open **MCP server** to begin setup. Existing OAuth clients must continue using discovery; do not replace their issuer URLs with the Console URL.
...
Add it as a custom connector using the URL above. The client registers itself, then opens Bravado’s authorization screen so you can sign in and approve the connection. Nothing is shared until you approve.
...
1. Start with [MCP in the Console](https://console.bravadotrade.com/endpoints/mcp); your client opens the discovered authorization endpoint.
...
<Note>
  **MCP requests carry a 10× multiplier.** Every MCP call counts as **10** against your request-rate budget, weighting each request for the heavier analytics it runs. A single tool call is therefore the equivalent of ten REST requests, so size your agent's polling and batch loops accordingly.
</Note>
