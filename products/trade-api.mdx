...
description: "The Bravado Trade API covers the full order lifecycle on supported prediction markets: standard and advanced order types, bracket orders, and position management including redeem, split, and merge."
icon:
...
| `PEGGED` | `record_id` | Automatically chases the touch price within a budget | Polymarket, Predict.fun |
| `ICEBERG` | `record_id` | Large order executed as hidden, repeated slices | Polymarket, Predict.fun |
...
<CardGroup cols={2}>
  <Card title="Polymarket" icon="/logo/polymarket.png" href="/markets/polymarket/trade-api">
    Standard and advanced orders, brackets, and positions (redeem/split/merge). Polygon, pUSD.
  </Card>
  <Card title="Predict.fun" icon="/logo/predict-fun.png" href="/markets/predict-fun/trade-api">
    LIMIT, MARKET, PEGGED, TWAP, ICEBERG + positions (redeem/split/merge/withdraw). BNB Chain, USDT.
  </Card>
</CardGroup>
...
| Order types | Standard, advanced and conditional; TRAILING_BUY is feature-gated | LIMIT, MARKET, PEGGED, TWAP, ICEBERG |
...
`TRAILING_BUY` is a Polymarket-only conditional entry capability. It requires the trailing-buy feature to be enabled in addition to the organisation’s trading access. It is not supported on Predict.fun. See the [order reference](/api/trade/orders) for the request schema; do not infer availability from a configured Console endpoint.
